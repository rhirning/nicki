...
public class EnterNameDialog extends CustomComponent {
	...
	public EnterNameDialog(String messageBase) {
		...
		createButton.addListener(new Button.ClickListener() {
			public void buttonClick(ClickEvent event) {
				if (getHandler().setName((String) name.getValue())) {
					getWindow().getParent().removeWindow(getWindow());
				} else {
					errorMessage.setValue("ungültiger Name");
				}
			}
		});
	...
	private AbsoluteLayout buildMainLayout() {
		...
		errorMessage.setWidth("-1px");
		errorMessage.setHeight("-1px");
		errorMessage.setImmediate(false);
		mainLayout.addComponent(errorMessage, "top:140.0px;left:20.0px;");
